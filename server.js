...
app.get('/player_info/post', function(req,res){
  var selected = req.query.player_choice;
  var idname = 'SELECT id, name FROM football_players;';
  var query1 = 'SELECT * FROM football_players WHERE id=' + selected + ';';
  var query2 = 'SELECT COUNT(*) FROM football_games WHERE (' + selected + ') =ANY(players);';
  db.task('get-everything', task=> {
    return task.batch([
      task.any(idname),
      task.any(query1),
